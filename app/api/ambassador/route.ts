...
const requestSchema = z.object({
  ...
  audienceType: z.string().min(1).max(80),
    ...
    try {
      await subscribeToList({
        ...
        properties: {
          ...
          AmbassadorHandle: parsed.handle,
          AmbassadorOtherLinks: parsed.otherLinks ?? null,
          AmbassadorAudienceType: parsed.audienceType,
  ...
  if (resendKey) {
    try {
      ...
      await resend.emails.send({
        ...
        subject: `[Ambassador] ${parsed.name} — ${parsed.audienceType}`,
        text: [
          `Name:           ${parsed.name}`,
          `Email:          ${parsed.email}`,
          `Country:        ${parsed.country}`,
          `Platform:       ${parsed.platform}`,
          `Handle/link:    ${parsed.handle}`,
          parsed.otherLinks ? `Other links:    ${parsed.otherLinks}` : null,
          `Audience type:  ${parsed.audienceType}`,
